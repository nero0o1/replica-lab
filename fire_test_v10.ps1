# Fire Test: 1:1 Forensic Comparison (DEBUG VERSION)
...
class ImporterV2 {
    ...
    [MvDocument] Import() {
        try {
            ...
            if ($xmlStart -lt 0) { throw "Root <editor> tag not found in $($this.Path)" }
            [xml]$xml = [xml]$contentString.Substring($xmlStart)
            
            $doc = [MvDocument]::new()
            $docNode = $xml.editor.item | Where-Object { $_.tableName -eq 'EDITOR_DOCUMENTO' }
            if ($null -eq $docNode) { throw "EDITOR_DOCUMENTO node not found" }
            ...
            foreach ($fn in $fieldNodes) {
                ...
                foreach ($pn in $props) {
                    $pId = [int]$pn.data.ROWSET.ROW.CD_PROPRIEDADE
                    $pval = $pn.data.ROWSET.ROW.LO_VALOR
                    $f.SetProperty([RosettaStone]::GetIdentifier($pId), $pval)
                }
                $doc.AddField($f)
            }
            return $doc
            ...
            throw "ImporterV2 Failure: $($_.Exception.Message)"
        }
    }
}
    ...
    [void] Export([MvDocument]$doc, [bool]$binary) {
        ...
        foreach ($f in $doc.Fields) {
            ...
            foreach ($k in $f.Properties.Keys) {
                $pId = [RosettaStone]::GetId($k)
                [void]$sb.AppendLine("        <item tableName='EDITOR_CAMPO_PROP_VAL'><data><ROWSET><ROW><CD_PROPRIEDADE>$pId</CD_PROPRIEDADE><LO_VALOR>$($f.Properties[$k])</LO_VALOR></ROW></ROWSET></data></item>")
            }
...
class DriverV3 {
    ...
    [void] Export([MvDocument]$doc) {
        $fields = @()
        foreach ($f in $doc.Fields) {
            $fields += @{ name = $f.Name; id = $f.IdLegacy }
        }
        $v3 = @{
            name    = $doc.Name
            fields  = $fields
            layouts = @(@{
                    name    = "Design"
                    content = (ConvertTo-Json @{ pageBody = @{ fields = $fields } } -Compress)
                })
        }
...
# --- 4. EXECUTION ---
try {
    Write-Host ">>> FIRE TEST START <<<" -ForegroundColor Cyan
    $original = "J:/replica_lab/11_unpack/edt2_runs/20260215_013504/907 - CHECKLIST DE SEGURANÇA CIRÚRGICA PRD_0053/05_documentos/tempfile0.edt"
    ...
    $doc = $imp.Import()
    Write-Host "[OK] Imported doc: $($doc.Name)" -ForegroundColor Green
    ...
    $genBytes = [System.IO.File]::ReadAllBytes($outV2)
    if ($genBytes[0] -eq 0xAC -and $genBytes[1] -eq 0xED) { Write-Host "[OK] V2 Magic Number: ACED0005" -ForegroundColor Green }
    
    $v3Json = Get-Content $outV3 -Raw | ConvertFrom-Json
    if ($v3Json.layouts[0].content -is [string]) { Write-Host "[OK] V3 Matrioska Rule (Double JSON) confirmed." -ForegroundColor Green }

    if ($doc.Fields.Count -gt 0) {
        $f = $doc.Fields[0]
        Write-Host "Sample Field: $($f.Name) (ID legacy: $($f.IdLegacy))"
        foreach ($k in $f.Properties.Keys) {
            if ($k -eq "acaoSql") { Write-Host "  -> Case ID 21 (acaoSql): '$($f.Properties[$k])' - SUCCESS" -ForegroundColor Green }
        }
    }
}
catch {
